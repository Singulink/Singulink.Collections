    ...
    public class ConcurrentDictionary<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary, IReadOnlyDictionary<TKey, TValue> where TKey : notnull
    {
        ...
        internal ConcurrentDictionary(int concurrencyLevel, int capacity, bool growLockArray, IEqualityComparer<TKey>? comparer)
        {
            if (concurrencyLevel <= 0)
            {
                ...
                {
                    throw new ArgumentOutOfRangeException(nameof(concurrencyLevel), "The concurrencyLevel argument must be positive, or -1 to indicate a default value.");
                }
        ...
        public bool TryRemove(KeyValuePair<TKey, TValue> item)
        {
            if (item.Key is null)
            {
                throw new ArgumentNullException(nameof(item), "TKey is a reference type and item.Key is null.");
            }
        ...
        {
            foreach (var x in _tables._countPerLock.AsSpan())
            {
                if (x != 0)
                {
                    return false;
        ...
        private static readonly ReadOnlyCollection<TKey> ROCTKeyEmpty = new ReadOnlyCollection<TKey>(Array.Empty<TKey>());
        private static readonly ReadOnlyCollection<TValue> ROCTValueEmpty = new ReadOnlyCollection<TValue>(Array.Empty<TValue>());
