...
/// <content>
/// Contains the AlternateLookup implementation for HashSetDictionary.
/// </content>
partial class HashSetDictionary<TKey, TValue>
{
    /// <summary>
    ...
    /// <summary>
    /// Provides an alternate lookup that can be used to perform operations on a <see cref="HashSetDictionary{TKey, TValue}"/> using <typeparamref
    ...
    public readonly struct AlternateLookup<TAlternateKey> where TAlternateKey : notnull, allows ref struct
    {
        ...
        private readonly Dictionary<TKey, ValueSet>.AlternateLookup<TAlternateKey> _altLookup;

        internal AlternateLookup(HashSetDictionary<TKey, TValue> dictionary, Dictionary<TKey, ValueSet>.AlternateLookup<TAlternateKey> altLookup)
        {
        ...
        /// </summary>
        public ValueSet this[TAlternateKey key]
        {
            get {
                ...
                return new ValueSet(_dictionary, Comparer.Create(key));
            }
        }
        ...
        {
            if (_altLookup.TryGetValue(key, out actualKey, out var valueSet))
            {
                DebugValid(valueSet);
        ...
        /// </summary>
        public int GetValueCount(TAlternateKey key) => _altLookup.TryGetValue(key, out var valueSet) ? valueSet.Count : 0;

        /// <inheritdoc cref="TryGetValues(TAlternateKey, out TKey, out ValueSet)"/>
        public bool TryGetValues(TAlternateKey key, [MaybeNullWhen(false)] out ValueSet valueSet) => _altLookup.TryGetValue(key, out valueSet);

        /// <summary>
        /// Gets the value set for the specified alternate key or <see langword="null"/> if the key was not found.
        /// </summary>
        /// <returns>A value indicating whether the key was found.</returns>
        public bool TryGetValues(TAlternateKey key, [MaybeNullWhen(false)] out TKey actualKey, [MaybeNullWhen(false)] out ValueSet valueSet) =>
            _altLookup.TryGetValue(key, out actualKey, out valueSet);
    }
}
