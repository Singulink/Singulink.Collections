...
/// Note: values of the array are assumed to be immutable and interchangeable (that is, the standard equality and hash code contracts must be held, but
...
public sealed class EquatableArray<T> : IReadOnlyList<T>, IList<T>, IEquatable<EquatableArray<T>>, IFormattable
{
    ...
    /// also must be consistent indefinitely, and the value you read out of the array is only guaranteed to be equal to the one passed in, not necessarily the
    /// same instance or bits).
    /// Note: this constructor does not de-duplicate against existing instances when created (however, it can still later) - it is a quick constructor that
    /// just copies the provided value directly, other than ensuring non-<see langword="null" /> - use methods on <see cref="EquatableArray" /> to get
    /// de-duplication.
    /// </summary>
